...
def delete_history_entry(history_id: int, db: Session = Depends(get_db)):
    entry = db.query(db_models.History).filter(db_models.History.id == history_id).first()
    if entry:
        db.delete(entry)
        db.commit()
    return {"success": True}
...
def clear_history(db: Session = Depends(get_db)):
    db.query(db_models.History).delete()
    db.commit()
    return {"success": True}
...
def export_history(db: Session = Depends(get_db)):
    items = db.query(db_models.History).order_by(desc(db_models.History.processed_at)).all()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow([
        'file_name', 'status', 'confidence', 'source_api',
        'artist', 'album', 'label_before', 'label_after',
        'genre_before', 'genre_after', 'skip_reason', 'error', 'processed_at'
    ])
    for r in items:
        writer.writerow([
